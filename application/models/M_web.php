...
class M_web extends CI_model{
	...
	public function pembayaran(){
		$this->db->query("UPDATE customer set saldo = ((SELECT saldo from customer join riwayat_login using(username_c) 
						where id_login = (SELECT max(id_login) from riwayat_login)) - (SELECT sum(harga_obat) 
						from pemesanan join customer using (username_c) join obat using (kode_obat) 
						where username_c = (select username_c from riwayat_login where id_login = (select max(id_login) 
						from riwayat_login)) and status = '')) where username_c = (SELECT username_c from riwayat_login 
						where id_login = (SELECT max(id_login) from riwayat_login))");

		$this->db->query("UPDATE pemesanan SET status = 'lunas' WHERE username_c = (select username_c from riwayat_login 
						where id_login = (select max(id_login) from riwayat_login))");
	}
